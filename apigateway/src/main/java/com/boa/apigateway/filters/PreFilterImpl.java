    ...
    public Object run() throws ZuulException {
	    ...
	    HttpEntity request = new HttpEntity<>(jwtRequest,headers);
		 ...
		 ResponseEntity<String> responseEntityStr = restTemplate.
          exchange(authUrl+"greet/userOrAdmin",HttpMethod.GET, request,
		  String.class); 
    ...
    private String parseString(String response)
		{
			...
			String token="";
		  	try {
				...
				int startIndex=response.indexOf("<token>");
				int endIndex=response.indexOf("</token>");
				String res = response.substring(startIndex+7, endIndex);
			 ...
			 * JsonFlattener.flattenAsMap(jsonObject.toString()); token=(String)
			 * flattenedJsonMap.get("token"); //log("\n=====Flatten As Map=====\n" +
			 * flattenedJson); // We are using Java8 forEach loop. More info:
			 * https://crunchify.com/?p=8047 //flattenedJsonMap.forEach((k, v) -> log(k +
			 * " : " + v));
			 * 
			 * // Unflatten it back to original JSON String nestedJson =
